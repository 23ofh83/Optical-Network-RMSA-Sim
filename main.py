...
def serve_request_protection(G, req):
    ...
    if not mod1 or not mod2: return False, []
...
def run_simulation(G_clean, traffic_file, algorithm_type, order_descending):
    ...
    for req in requests:
        if algorithm_type == '1+1 Protection (First-Fit)':
            success, chunks_info = serve_request_protection(G, req)
        else:
            paths_info = get_k_shortest_paths(G, req["source"], req["destination"], k=5)
            if algorithm_type == 'Benchmark':
                success, chunks_info = serve_request_benchmark(G, paths_info, req)
            else:
...
if __name__ == "__main__":
    ...
    algorithms = ['Benchmark', 'Custom(NoC-aware Best-Fit)', '1+1 Protection (First-Fit)']
        ...
        for i in range(1, 6):
            ...
            for algo in algorithms:
                for order_name, is_desc in orders:
                    ...
                    print(f"{network['name']:<5} | M{i:<4} | {algo:<26} | {res['alloc']}/{res['blk']:<9} | {res['bp']:<6.2f} | {res['highest_fsu']:<7} | {res['total_fsu']:<8} | {res['avg_len']:<7.1f} | {res['cost']:<7.1f} | {res['rss']:<6.1f} | {res['noc']:<6}")
